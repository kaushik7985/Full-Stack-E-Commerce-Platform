                            ...
                            userOrder?.map((order)=>(
                                <div key={order._id} className='shadow-lg rounded-2xl p-5 border border-gray-200'>
                                    ...
                                    <div className='flex justify-between items-center'>
                                        ...
                                        "Failed" ? "bg-red-500":"bg-orange-300"} text-white px-2 py-1 rounded-lg`}>
                                        {order.status}</span>
                                    </div>
                                        ...
                                        <ul className='space-y-2'>
                                            {
                                                order.products.map((product, index)=>(
                                                    ...
                                                    bg-gray-50 p-3 rounded-lg'>
                                                        <img onClick={()=>navigate(`/products/${product?.
                                                        productId?._id}`)}
                                                        className='w-16 cursor-pointer' src={product.productId?.
                                                        productImg?.[0].url} alt=""/>
                                                        <span className='w-[300px] line-clamp-2'>{product.productId?.
                                                        productName}</span>
